...
### Purpose
This Streamlit application serves as an interactive development blueprint for Software Developers and Data Engineers. It guides them through establishing a robust, scalable, and maintainable foundational setup for new AI services, based on the principles demonstrated in the "Individual AI-Readiness Platform" case study. The application focuses on applying best practices in Python development, API design with FastAPI, Pydantic for configuration, and comprehensive health checks, simulating a real-world workflow.

### High-Level Story Flow

The application walks the learner through critical phases of setting up a production-ready AI service skeleton:

1.  **Introduction**: A welcoming page introduces the "Individual AI-Readiness Platform" project and outlines the objectives for establishing a robust AI service foundation.
2.  **Project Initialization (Task 1.1)**: Users "initialize" a new Python project using Poetry and establish a standard directory structure for an API-driven AI service.
3.  **Configuration System with Full Validation (Task 1.2)**: The application demonstrates how to implement a fully validated configuration system using `pydantic-settings`, showcasing how to prevent common configuration errors with `model_validator`.
4.  **FastAPI Application with Versioned Routers (Task 1.3)**: Users build the core FastAPI application, integrating versioned API routes and crucial middleware for concerns like CORS, request ID tracking, and custom exception handling.
5.  **Health Check with Dependency Status (Task 1.4)**: The app illustrates how to build comprehensive health check endpoints (basic, detailed, readiness, liveness) that monitor the application and its critical dependencies.
6.  **Common Mistakes & Troubleshooting**: A final section reviews common pitfalls in production service setup and demonstrates how the adopted architectural patterns proactively address them, reinforcing best practices.
...
from source import (
    ...
    create_app_notebook,
    health_check_func,
    detailed_health_check_func,
    readiness_check_func,
    liveness_check_func,
    setup_tracing, # Placeholder tracing function
    # Note: v1_router, v2_router, health_router and _startup_time are defined in source.py
    # but their direct manipulation/inclusion for Streamlit demo is handled via create_app_notebook
    # and subsequent calls. Their values might be implicitly global in source.py if imported with `*`.
)
...
*   `st.session_state.current_page`:
    ...
    *   **Update**: When the user selects a new page from the sidebar dropdown. The app reruns to display the selected content.
    *   **Read**: Controls which section of the main content area is rendered using conditional logic (`if/elif`).
*   `st.session_state.settings_object`:
    *   **Initialization**: `st.session_state.settings_object = None`
    *   **Update**: Set to the `Settings` instance returned by `get_settings()` (from `source.py`) after the user clicks the "Load and Validate Settings" button in "Task 1.2: Configuration System".
    *   **Read**: Passed as an argument to `create_app_notebook()`, `health_check_func()`, `detailed_health_check_func()`, `readiness_check_func()`, and `simulate_app_lifespan_output_async()` for subsequent tasks, ensuring consistent configuration across the application flow.
*   `st.session_state.fastapi_app_object`:
    *   **Initialization**: `st.session_state.fastapi_app_object = None`
    *   **Update**: Set to the `FastAPI` app instance returned by `create_app_notebook(st.session_state.settings_object)` (from `source.py`) after the user clicks "Create FastAPI Application" in "Task 1.3: FastAPI Application".
    *   **Read**: Passed to `simulate_app_lifespan_output_async()` and used conceptually in "Task 1.4: Health Check" and "Common Mistakes & Troubleshooting" to represent the running API.
*   `st.session_state.project_init_output`:
    *   **Initialization**: `st.session_state.project_init_output = None`
    *   **Update**: Stores the string output from `simulate_project_initialization_output()` after the "Simulate Project Initialization" button is clicked.
    *   **Read**: Displayed as a code block in "Task 1.1: Project Initialization".
*   `st.session_state.config_validation_output`:
    *   **Initialization**: `st.session_state.config_validation_output = None`
    *   **Update**: Stores the formatted string representation of the loaded `settings_object` after the "Load and Validate Settings" button is clicked.
    *   **Read**: Displayed as a code block in "Task 1.2: Configuration System".
*   `st.session_state.fastapi_app_output`:
    *   **Initialization**: `st.session_state.fastapi_app_output = None`
    *   **Update**: Stores the string output from `simulate_app_lifespan_output_async()` after the "Create FastAPI Application" button is clicked.
    *   **Read**: Displayed as a code block in "Task 1.3: FastAPI Application".
*   `st.session_state.basic_health_output`, `st.session_state.detailed_health_output`, `st.session_state.readiness_output`, `st.session_state.liveness_output`:
    *   **Initialization**: All set to `None`.
    *   **Update**: Store the JSON/plaintext string output from respective health check functions (`health_check_func`, `detailed_health_check_func`, `readiness_check_func`, `liveness_check_func`) after corresponding buttons are clicked.
    *   **Read**: Displayed as code blocks on "Task 1.4: Health Check".
*   `st.session_state.mistake1_output`, `st.session_state.mistake2_output`, `st.session_state.mistake3_output`:
    *   **Initialization**: All set to `None`.
    *   **Update**: Store the string output from `simulate_bad_settings_load_output()`, `simulate_secret_str_handling_output()`, and `simulate_app_lifespan_output_async()` (when triggered as a mistake demo) after their respective "Demonstrate" buttons are clicked.
    *   **Read**: Displayed as code blocks on "Common Mistakes & Troubleshooting".

### Application Layout, Widgets, and Interaction
...
page_selection = st.sidebar.selectbox(
    ...
    index=['Introduction', 'Task 1.1: Project Initialization', 'Task 1.2: Configuration System', 'Task 1.3: FastAPI Application', 'Task 1.4: Health Check', 'Common Mistakes & Troubleshooting'].index(st.session_state.current_page)
)
